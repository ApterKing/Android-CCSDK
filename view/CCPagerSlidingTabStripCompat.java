 ...
 * 参考网络awsome-android-ui 修改
 * 用于{@link CCViewPagerCompat}顶部可滑动的选项块，使用方式类似于android.support.v4.view.PagerTabStrip
 * 代码中：
 *      CCPagerSlidingTabStripCompat tabStrip = ...
 *      ViewPager pager = ...
 *      pager.setAdapter(adapter);
 *      tabStrip.setViewPager(pager)
 *
 *      // 另： PagerAdapter implements CCPagerTabProvider 可以自定义每个Tab 显示的样式， 默认只显示一个TextView
 * Res:
 *     参数参考 attrs
 *
 */
public class CCPagerSlidingTabStripCompat extends HorizontalScrollView {

    public interface CCPagerTabProvider {
    ...
    private OnTabSelectedListener tabReselectedListener = null;
    public ViewPager.OnPageChangeListener delegatePageListener;

    private LinearLayout tabsContainer;
    private CCViewPagerCompat pagerCompat;
    ...
    public CCPagerSlidingTabStripCompat(Context context) {
        this(context, null);
    }

    public CCPagerSlidingTabStripCompat(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CCPagerSlidingTabStripCompat(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setViewPagerCompat(CCViewPagerCompat pagerCompat) {
        this.pagerCompat = pagerCompat;
        if (pagerCompat.getAdapter() == null) {
            throw new IllegalStateException("ViewPager does not have adapter instance.");
        }

        pagerCompat.setOnPageChangeListener(pageListener);
        pagerCompat.getAdapter().registerDataSetObserver(adapterObserver);
     ...
     * @return
     */
    private TextView generateDefaultTabView() {
...
//        textView.setTextIsSelectable(true);
     ...
     * @return
     */
    private StateListDrawable generateDefaultTabBackground() {
        if (tabBackgroundDrawable == null) {
            tabBackgroundDrawable = new StateListDrawable();
            tabBackgroundDrawable.addState(new int[]{android.R.attr.state_focused, android.R.attr.state_pressed},
                    new ColorDrawable(Color.argb(0xFF, 0x1A, 0xFF, 0xFF)));
            tabBackgroundDrawable.addState(new int[]{-android.R.attr.state_focused, -android.R.attr.state_pressed},
                    new ColorDrawable(Color.argb(0x00, 0x00, 0x00, 0x00)));
        }
        return tabBackgroundDrawable;
    }

    public void notifyDataSetChanged() {
        tabsContainer.removeAllViews();
        tabCount = pagerCompat.getAdapter().getCount();
        View tabView;
        for (int i = 0; i < tabCount; i++) {

            if (pagerCompat.getAdapter() instanceof CCPagerTabProvider) {
                tabView = ((CCPagerTabProvider) pagerCompat.getAdapter()).getCustomTabView(this, i);
            } else {
                // 修改这里
                tabView = generateDefaultTabView();
            }

            CharSequence title = pagerCompat.getAdapter().getPageTitle(i);
    ...
    private void addTab(final int position, CharSequence title, View tabView) {
        ...
        tabView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (pagerCompat.getCurrentItem() != position) {
                    View tab = tabsContainer.getChildAt(pagerCompat.getCurrentItem());
                    notSelected(tab);
                    pagerCompat.setCurrentItem(position);
    ...
    private void updateTabStyles() {
        for (int i = 0; i < tabCount; i++) {
            ...
            if (tabBackgroundResId != -1) {
                ...
                v.setBackgroundDrawable(generateDefaultTabBackground());
            }
...
//            TextView tab_title = (TextView) v.findViewById(R.id.psts_tab_title);
            ...
            // 修改这里,如果采用默认的则直接设置title
            TextView tab_title = null;
            if (v instanceof TextView)
                tab_title = (TextView) v;
    ...
    private ViewTreeObserver.OnGlobalLayoutListener firstTabGlobalLayoutListener = new ViewTreeObserver.OnGlobalLayoutListener() {
        ...
        public void onGlobalLayout() {
            ...
            currentPosition = pagerCompat.getCurrentItem();
    ...
    public void setOnTabSelectedListener(OnTabSelectedListener tabReselectedListener) {
    ...
    private class PageListener implements CCViewPagerCompat.OnPageChangeListener {
        ...
        public void onPageScrollStateChanged(int state) {
            if (state == ViewPager.SCROLL_STATE_IDLE) {
                scrollToChild(pagerCompat.getCurrentItem(), 0);
            }
            //Full textAlpha for current item
            View currentTab = tabsContainer.getChildAt(pagerCompat.getCurrentItem());
            selected(currentTab);
            //Half transparent for prev item
            if (pagerCompat.getCurrentItem() - 1 >= 0) {
                View prevTab = tabsContainer.getChildAt(pagerCompat.getCurrentItem() - 1);
                notSelected(prevTab);
            }
            //Half transparent for next item
            if (pagerCompat.getCurrentItem() + 1 <= pagerCompat.getAdapter().getCount() - 1) {
                View nextTab = tabsContainer.getChildAt(pagerCompat.getCurrentItem() + 1);
                notSelected(nextTab);
            }
...
//            TextView title = (TextView) tab.findViewById(R.id.psts_tab_title);
    ...
    protected void onDetachedFromWindow() {
        ...
        if (pagerCompat != null) {
            if (adapterObserver.isAttached()) {
                pagerCompat.getAdapter().unregisterDataSetObserver(adapterObserver);
